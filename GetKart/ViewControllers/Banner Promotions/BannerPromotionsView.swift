                    ...
                    HStack {
                        ...
                        VStack(alignment: .leading) {
                            ...
                            Text("Location & Radius")
                                .font(.manrope(.semiBold, size: 15.0))
                            
                            if strAddress.count > 0 {
                                
                                Text("\(strAddress) / \(radius)km")
...
//        }
    ...
    func pushToLocationcreen() {
        ...
        let destVC = UIHostingController(
            rootView: ChooseLocationBannerView(
                navigationController: self.navigationController,
                minSliderValue: settingObj.min_radius,
                maxSliderValue: settingObj.max_radius,
                defaultValue: Int(Double(settingObj.default_radius))
            ) { (
                lat,
                long,
                address,
