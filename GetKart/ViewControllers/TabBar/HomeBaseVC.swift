...
//        let vc2 = UINavigationController(rootViewController: boardVc)
    ...
    // ✅ FIXED: badge x now calculated from the RIGHT edge of the Board tab
    //
    // Visual target (from screenshot):
    //   NEW badge sits at top-right corner of the Board icon,
    //   half overlapping the icon's right edge, a few px above icon top
    //
    // Calculation:
    //   tabWidth       = tabBar.width / 5
    //   tabRightEdge   = tabWidth * 1           → right edge of tab 0
    //   iconWidth      ≈ 28pt
    //   iconRightEdge  = tabRightEdge - ((tabWidth - iconWidth) / 2)
    //                  = tabRightEdge - margin   → where icon's right edge actually is
    //   badgeX         = iconRightEdge - badgeWidth + 4   → badge right edge slightly past icon right
    ...
    /*func addNewBadgeOnBoardTab() {
        ...
        let totalTabs     = items.count                              // 5
        let tabWidth      = tabBar.bounds.width / CGFloat(totalTabs) // width of one tab slot
        let tabRightEdge  = tabWidth                                 // right edge of Board tab (index 0)

        let iconWidth:    CGFloat = 28   // standard tab icon size
        let badgeWidth:   CGFloat = 22   // wide enough for NEW image
        let badgeHeight:  CGFloat = 12

        // margin between tab edge and icon edge (icon is centred in the tab)
        let iconMargin    = (tabWidth - iconWidth) / 2
        let iconRightEdge = tabRightEdge  - iconMargin                // actual right edge of the icon

        // ✅ place badge so its center sits on the icon's right edge
        let badgeX = iconRightEdge + (badgeWidth / 2)
        let badgeY: CGFloat = 10   // near top of tab bar, above icon
...
//    override func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
//
//        // If touch lands inside middleButton (including y < 0 overflow above tabBar), route to it
...
//        }
//
//        // For all other touches let super handle normally
//        return super.hitTest(point, with: event)
//    }
//    
    override func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
