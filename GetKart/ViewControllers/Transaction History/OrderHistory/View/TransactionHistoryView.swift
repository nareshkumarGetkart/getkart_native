    ...
    var body: some View {
        HStack(spacing:5) {
            ...
            AsyncImage(url: URL(string: transaction.package?.icon ?? "")) { img in
                ...
                if (transaction.walletTransaction?.title.count ?? 0) > 0{
                    Image("wallet").resizable().aspectRatio(contentMode: .fit).frame(width:50,height: 50).background(Color(hex: "#FEF6E9")).cornerRadius(12)
                }else{
            ...
            VStack(alignment: .leading, spacing: 5) {
                ...
                if (transaction.package?.name ?? "").count > 0{
                  
                    Text(transaction.package?.name ?? "")
                        .font(Font.inter(.semiBold, size: 16))
                        .foregroundColor(Color(.label))
                    
                }else if (transaction.walletTransaction?.title.count ?? 0) > 0{
                   
                    Text(transaction.walletTransaction?.title ?? "")
                        .font(Font.inter(.semiBold, size: 16))
                        .foregroundColor(Color(.label))
                    
                }else if (transaction.package?.name ?? "").count == 0 && ((transaction.transactionPackage?.name?.count ?? 0) > 0){
