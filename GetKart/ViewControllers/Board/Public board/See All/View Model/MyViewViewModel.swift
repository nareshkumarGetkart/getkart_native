...
class MyViewViewModel:ObservableObject{
    ...
    private func fetchBoards(showLoader: Bool = false) {
        ...
        var url =
        Constant.shared.get_views_items + "?page=\(page)"

        if seeallType == .featured{
            url =
            Constant.shared.get_featured_board + "?page=\(page)"
        }else if seeallType == .ideasforyou{
            url =
            Constant.shared.get_ideas_foru + "?page=\(page)"
        }else if seeallType == .popular{
            url =
            Constant.shared.get_popular_items + "?page=\(page)"
        }
