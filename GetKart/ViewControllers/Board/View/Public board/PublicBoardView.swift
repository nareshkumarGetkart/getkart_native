...
//                .foregroundColor(.gray)
            Text("Search any item...").lineLimit(1)
                .foregroundColor(.gray)
            Spacer()
        }
        .padding(.vertical, 8)
...
struct CardItemView: View {
    ...
    var body: some View {
        ...
        if item.boardType == 1 {
            ...
            //Idea View
            IdeaCardStaggered(product: item).onTapGesture(perform: onTap)
        } else if item.boardType == 4 {
            // Full-width horizontal banner — NEW
            HorizontalBannerCard(product: item, onTapButton: onTapBoostButton)
        } else {
            ProductCardStaggered(product: item, sendLikeUnlikeObject: onLike, onTapBoostButton: onTapBoostButton, isToShowBoostButton: isToShowBoostButton)
                .onTapGesture(perform: onTap)
        }
...
// MARK: - HorizontalBannerCard  (NEW)
// Rendered when boardType == 4. Spans full scroll-view width.
// Image uses scaledToFill so it always fills the banner area.
// CTA bar identical to PromotionalAdsCardStaggered.
...
struct HorizontalBannerCard: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            KFImage(URL(string: product.image ?? ""))
                .setProcessor(
                    DownsamplingImageProcessor(size: CGSize(width: 800, height: 400))
                )
                .scaleFactor(UIScreen.main.scale)
                .cacheOriginalImage(false)
                .resizable()
