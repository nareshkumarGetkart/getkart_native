...
struct CreateBoardView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showSheet) {
         ...
         .sheet(isPresented: $showBoostSheet) {
             BoostBoardPlanView(categoryId:selectedCategoryId ?? 0,packageSelectedPressed: { (selPkgObj,selPaymentMethod)  in
    ...
    func createBoardApi(selPkgObj:PlanModel? = nil,selPaymentMethod:SelPaymentMethod = .other){
                            ...
                            do{
                                ...
                                if let itemObj = item.data?.first{
                                    ...
                                    if let pkgObj = selPkgObj{
                                        self.paymentGatewayOpen(selPlan: pkgObj, item: itemObj,selPaymentMethod: selPaymentMethod)
                                    }
                                }
                            }catch{
