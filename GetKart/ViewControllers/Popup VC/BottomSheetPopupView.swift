        ...
        VStack(spacing: 15) {
            ...
            VStack(spacing: 12) {
                if let title = objPopup.title {
                    ...
                    if title.isHTML{
                        HTMLOrTextView(htmlContent: title, defaultFont:UIFont(name: "Manrope-SemiBold", size: 17.0) ?? UIFont.systemFont(ofSize: 17, weight: .medium))
                ...
                if let subtitle = objPopup.subtitle {
                    ...
                    if subtitle.isHTML{
                        HTMLOrTextView(htmlContent: subtitle, defaultFont:UIFont(name: "Manrope-Medium", size: 16.0) ?? UIFont.systemFont(ofSize: 16, weight: .medium))
                            .fixedSize(horizontal: false, vertical: true)
                    }else{
...
func extractHTMLColor(_ html: String) -> Color {
    ...
    if let range = html.range(of: #"color:\s*(#[0-9A-Fa-f]{6})"#, options: .regularExpression),
       let hex = html[range].split(separator: "#").last {
        ...
        return Color(hex: String(hex)) ?? .black
    }
    return .black
}
