...
class VKImageZoom: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        zoom_imgView?.contentMode = .scaleAspectFill
        img_scroll?.contentMode = .scaleAspectFit
        // add components...
    ...
    private func addView_components() -> Void {
        ...
        cancel_btn?.setImage(UIImage(named: "close-small"), for: .normal)
        cancel_btn?.titleLabel?.font = UIFont.systemFont(ofSize: 15.0)
        cancel_btn?.setTitleColor(UIColor.red, for: .normal)
        ...
        cancel_btn?.layer.borderColor = UIColor.red.cgColor
        
        // create indicator...
        if act_indicator != nil {
            act_indicator?.removeFromSuperview()
        }
        act_indicator = UIActivityIndicatorView(style: .white)
        ...
        img_scroll?.decelerationRate = UIScrollView.DecelerationRate.fast
    }
...
extension VKImageZoom {
    ...
    func loadingImages() {
        ...
        if self.image != nil {
            ...
            zoom_imgView?.center = (img_scroll?.center)!
            zoom_imgView?.contentMode = .scaleAspectFit
        }
            ...
            { (data, response, error) in
                ...
                DispatchQueue.main.async {
                    if let load_image = UIImage(data: data!) {
                        ...
                        self.zoom_imgView?.contentMode = .scaleAspectFit
    ...
    func imageWidth_height(image: UIImage?) -> (width: CGFloat, height: CGFloat) {
        ...
        // image height & weight...
        var imgWidth =  self.view.frame.size.width // image?.size.width ?? CGFloat(0)
        var imgHeight =  self.view.frame.size.height // image?.size.height ?? CGFloat(0)
