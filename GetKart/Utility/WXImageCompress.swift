...
public extension UIImage {
    ...
    /**
     ...
     - returns: thumb image
     */
    func wxCompress(type: WechatCompressType = .timeline,compressionQuality:CGFloat=0.5) -> UIImage {
        let size = self.wxImageSize(type: type)
        let reImage = resizedImage(size: size)
        let data = reImage.jpegData(compressionQuality: compressionQuality)!
        print(" After compress size of image in MB: %f ", Double(data.count) / 1024.0/1024.0)
